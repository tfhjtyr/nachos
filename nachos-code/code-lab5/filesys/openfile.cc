...
{ 
	...
	if(name != NULL) {
		...
		FID = directory->getFID(name);
		delete directory;
		//printf("FID: %d\n", FID);
		if(SR[FID] == NULL)
			SR[FID] = new Lock("file read lock");
		if(SW[FID] == NULL)
			SW[FID] = new Lock("file write lock");
		if(SD[FID] == NULL) {
			SD[FID] = new Lock("file delete lock");
			SD[FID]->Acquire();
		}
	}
	else
...
OpenFile::Read(char *into, int numBytes)
{
	...
	if(readFile != NULL) 
		result = readFile->ReadAt(into, numBytes, 0);
	else
...
OpenFile::Write(char *into, int numBytes)
{
	...
	if(writeFile != NULL)
		result = writeFile->WriteAt(into, numBytes, 0);
	else
...
{		
	//printf("In ReadAt()\n");
	if(FID != -1) {
		printf("SW[%d] Acquire() by %s\n", FID, currentThread->getName());
		SW[FID]->Acquire();
		printf("SW[%d] passes Acquire() by %s\n", FID, currentThread->getName());
	}
	//currentThread->Yield();
    ...
    if ((numBytes <= 0) || (position >= fileLength)) {
		if(FID != -1) {
			printf("SW[%d] Release() by %s\n", FID, currentThread->getName());
			SW[FID]->Release();
			printf("SW[%d] passes Release() by %s\n", FID, currentThread->getName());
		}
		//printf("Out ReadAt()\n");
    	return 0; 	
	}
...
OpenFile::WriteAt(char *from, int numBytes, int position)
{
	//printf("In WriteAt()\n");
	if(FID != -1) {
		printf("SR[%d] Acquire() by %s\n", FID, currentThread->getName());
		SR[FID]->Acquire();
		printf("SR[%d] passes Acquire() by %s\n", FID, currentThread->getName());
		printf("SW[%d] Acquire() by %s\n", FID, currentThread->getName());
		SW[FID]->Acquire();
		printf("SW[%d] passes Acquire() by %s\n", FID, currentThread->getName());
	}
	...
	//printf("i'm here\n");
	if(fileName != NULL) {
		//printf("%s, %s, %s\n", fileName, fileSystem->directoryFile->fileName, fileSystem->formerDirectory->fileName);
		Directory* directory = new Directory(NumDirEntries);
   	 	directory->FetchFrom(fileSystem->directoryFile);
	...
	if ((position + numBytes) > fileLength) {
		...
		hdr->allocateMore(freeMap,	position + numBytes - fileLength);
...
// write modified sectors back
	...
	//printf("OutPuting\n");
	if(FID != -1) {
		printf("SR[%d] Release() by %s\n", FID, currentThread->getName());
		SR[FID]->Release();
		printf("SR[%d] passes Release() by %s\n", FID, currentThread->getName());
		printf("SW[%d] Release() by %s\n", FID, currentThread->getName());
		SW[FID]->Release();
		printf("SW[%d] passes Release() by %s\n", FID, currentThread->getName());
	}
...
void 
OpenFile::redirectReadFile(char * name)
{
	readFile = fileSystem->Open(name);
}

void 
OpenFile::redirectWriteFile(char * name)
{
	writeFile = fileSystem->Open(name);
}