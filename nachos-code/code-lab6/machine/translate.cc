...
// from the virtual address
    ...
    if (tlb == NULL) {		// => page table => vpn is index into table
		...
		DEBUG('a', "Searching in tlb...\n");
        for (entry = NULL, i = 0; i < TLBSize; i++) {
    	    if (tlb[i].valid && (tlb[i].virtualPage == vpn)) {
				entry = &tlb[i];			// FOUND!
			//	printf("\t\t\tTLB hit at virtual page number: %d\n", vpn);
				break;
	    	}
        }
		if (entry == NULL) {				// not found
    	   	DEBUG('a', "*** no valid TLB entry found for this virtual page!\n");
		//	printf("TLB miss at virtual page number: %d\n", vpn);
	...
	memBitMap->myEntry[pageFrame]->lastUsedTime = stats->totalTicks;
	
	entry->lastUsedTime = stats->totalTicks;
	entry->numOfReference++;
