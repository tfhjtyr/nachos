...
{
	//printf("In console::putchar %c\n", ch);
    ASSERT(putBusy == FALSE);
    WriteFile(writeFileNo, &ch, sizeof(char));
    putBusy = TRUE;
    interrupt->Schedule(ConsoleWriteDone, (int)this, ConsoleTime,
					ConsoleWriteInt);
	//printf("Leaving console::putchar %c\n", ch);
}


static void dummyReadAvail(int arg)
{
	SynchConsole *synchConsole = (SynchConsole*) arg;
	synchConsole->ReadAvail(0);
}

static void dummyWriteDone(int arg)
{
	SynchConsole *synchConsole = (SynchConsole*) arg;
	synchConsole->WriteDone(0);
}

SynchConsole::SynchConsole(char *readFile, char *writeFile)
{
	printf("initiating synchConsole...\n");
	lock = new Lock("console lock");
	readAvail = new Semaphore("readAvail semaphore", 0);
	writeDone = new Semaphore("writeDone semaphore", 0);
	console = new Console(readFile, writeFile, dummyReadAvail, dummyWriteDone, (int)this);
	printf("initiated synchConsole successfully...\n");
}
...
{
	printf("Puting a char %c\n", ch);
	lock->Acquire();
	console->PutChar(ch);
	printf("\nwriteDone P()\n");
	writeDone->P();
	lock->Release();
	printf("Put a char %c successfully...\n", ch);
}

char
SynchConsole::GetChar()
{
	printf("Getting a char %c\n");
	char ch;
	lock->Acquire();
	printf("readAvail P()\n");
	readAvail->P();
	ch = console->GetChar();
	lock->Release();
	printf("Got a char %c successfully...\n", ch);
	return ch;
}